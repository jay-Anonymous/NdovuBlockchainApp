...
)
{
	...
	ReferenceCollector collector(*_declaration, _sourceOffset, _sourceIdentifierName);
...
)
{
	...
	if (auto const* identifier = dynamic_cast<Identifier const*>(_sourceNode))
		references += collect(identifier->annotation().referencedDeclaration, _sourceOffset, _sourceUnit, identifier->name());
	else if (auto const* identifierPath = dynamic_cast<IdentifierPath const*>(_sourceNode))
	{
		...
		for (size_t i = 0; i < identifierPath->pathLocations().size(); ++i)
		{
			...
			if (location.containsOffset(_sourceOffset))
			{
				...
				references += collect(declaration, _sourceOffset, _sourceUnit, name);
				break;
			}
		}
	}
	else if (auto const* memberAccess = dynamic_cast<MemberAccess const*>(_sourceNode))
		references += collect(memberAccess->annotation().referencedDeclaration, _sourceOffset, _sourceUnit, memberAccess->memberName());
